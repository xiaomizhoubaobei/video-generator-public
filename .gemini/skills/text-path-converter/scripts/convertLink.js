...
function convertLink(text, repoSlug, branchOrSha) {
  ...
  if (inlineCodeBlockPositions.length % 2 !== 0) {
    inlineCodeBlockPositions = inlineCodeBlockPositions.slice(
      0,
      inlineCodeBlockPositions.length - 1,
    );
  }

  // 整理起始和结束位置
  const excludedRanges = [...codeBlockPositions, ...inlineCodeBlockPositions].reduce(
    (result, item, index) => {
      if (index % 2 === 0) {
