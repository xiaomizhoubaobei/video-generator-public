...
export async function getVideoModels(): Promise<VideoModelSchemaResponseType> {
  const response = await apiKy.get(VIDEO_MODELS_API_URL).json<VideoModelSchemaResponseType>();
...
const filterVideoModel = (
  ...
  return {
    ...
    models: modelInfo.models.filter((model) => !modelNames.includes(model.model_name)),
  };
};
