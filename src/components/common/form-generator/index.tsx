...
export type FormGeneratorProps<T extends FieldValues> = {
  ...
  options?: { value: string; label: string; id: string; disabled?: boolean }[];
  placeholder?: string;
  label?: string;
  lines?: number;
...
const FormGenerator = <T extends FieldValues>({
  ...
  // Sync select defaultValue to form when options change (must be at top level)
  useEffect(() => {
    if (inputType === "select") {
      const watchSelect = watch(name, undefined) as string;
      if (!watchSelect && defaultValue && options?.length) {
    ...
    <TooltipProvider>
      <Tooltip delayDuration={300}>
        <TooltipTrigger asChild>
          <button
            type="button"
            className="cursor-help border-none bg-transparent p-0"
        ...
        <TooltipContent
          sideOffset={4}
          className="max-w-xs rounded-md bg-gray-900 px-3 py-2 text-sm break-words text-gray-50 select-text"
        >
          <p>{content}</p>
        </TooltipContent>
      </Tooltip>
  ...
  const renderTextarea = () => {
    ...
    const renderBottomContent = () => {
      ...
      return (
        <div className="bg-background flex items-end justify-between p-2">
    ...
    return (
      <Label className="relative flex flex-col gap-2" htmlFor={`input-${id}`}>
        <Button
          type="button"
          variant="ghost"
          size="icon"
          ...
          className="absolute right-0 bottom-0 hover:scale-120 hover:bg-transparent"
        >
