...
export function ImageGenerator({ open = false, onOpenChange, onImageSelect }: ImageGeneratorProps) {
      ...
      {isMobile ? (
        <Drawer open={open && !fullscreenImageUrl} onOpenChange={onOpenChange}>
          <DrawerContent className="flex h-[80vh] flex-col">
            <DrawerHeader>
              <DrawerTitle className="text-normal">{t("image_generator_title")}</DrawerTitle>
            </DrawerHeader>
            <Tabs
              value={activeTab}
            ...
            >
              <TabsList className="mx-6 mb-4 grid grid-cols-2">
                <TabsTrigger value="form">{t("image_generator_title")}</TabsTrigger>
                <TabsTrigger value="history">{t("history")}</TabsTrigger>
              </TabsList>

              <TabsContent value="form" className="m-0 flex min-h-0 flex-1 flex-col px-6 pb-6">
                <ImageGeneratorForm />
              </TabsContent>
              <TabsContent value="history" className="m-0 min-h-0 flex-1 overflow-y-auto px-6 pb-6">
                {tasks.length === 0 ? (
                  <EmptyInterface />
                ) : (
                  <Masonry
